...
async function captureEvidence(page, url, extractedData) {
  ...
  const evidence = { screenshots: [], timestamp };

  // 1) 全页截图
  const fullPath = join(snapshotsDir, timestamp + '_full.png');
  await page.screenshot({ path: fullPath, fullPage: true });
  evidence.screenshots.push({ id: 'full', path: '/snapshots/' + timestamp + '_full.png', label: '页面全貌', description: '网页首屏及全页完整截图' });

  // 2) 首屏 Hero 区域截图
  const heroPath = join(snapshotsDir, timestamp + '_hero.png');
  await page.screenshot({ path: heroPath, fullPage: false });
  evidence.screenshots.push({ id: 'hero', path: '/snapshots/' + timestamp + '_hero.png', label: '首屏 Hero', description: '首屏视口区域 — 通常包含主色、标题字体、核心视觉元素' });

  // 3) 对关键元素逐个截图并标注
  const selectors = [
    { id: 'heading', selector: 'h1', label: '标题字体', descFn: (d) => '字体：' + (d.fonts?.[0]?.[0] || '未知') + ' — 来源于页面 <h1> 元素' },
    { id: 'nav', selector: 'nav, header nav, [class*="nav"]', label: '导航栏', descFn: () => '导航栏结构 — 包含边框、间距、字体等风格线索' },
    { id: 'button', selector: 'button, a[class*="btn"], a[class*="button"], [class*="cta"]', label: '按钮组件', descFn: (d) => '按钮样式 — 圆角、背景色、字重等来源依据' },
    { id: 'card', selector: '[class*="card"], article, [class*="item"]', label: '卡片/内容块', descFn: () => '卡片组件 — 圆角、阴影、边框等来源依据' },
    { id: 'footer', selector: 'footer', label: '页脚', descFn: () => '页脚区域 — 次色/柔和色来源依据' }
  ];

  for (const { id, selector, label, descFn } of selectors) {
    try {
      ...
      await page.evaluate((sel) => {
        ...
        if (el) {
          el.dataset.ssOrigOutline = el.style.outline || '';
          el.style.outline = '3px solid #6366f1';
          el.style.outlineOffset = '2px';
        }
      }, selector);

      const elemPath = join(snapshotsDir, timestamp + '_' + id + '.png');
      const padding = 20;
      ...
      await page.evaluate((sel) => {
        const el = document.querySelector(sel);
        if (el) { el.style.outline = el.dataset.ssOrigOutline || ''; delete el.dataset.ssOrigOutline; }
      }, selector);

      evidence.screenshots.push({ id, path: '/snapshots/' + timestamp + '_' + id + '.png', label, description: descFn(extractedData) });
    } catch (err) {
      // 跳过不可截图的元素
    }
  }

  // 4) 颜色来源标注（在全页截图上，收集颜色出现位置）
  const colorSources = await page.evaluate(() => {
    const sources = [];
    const seen = new Set();
    const elements = document.querySelectorAll('*');
    for (const el of elements) {
      ...
      sources.push({ color: bg, tag: tagInfo, rect: { x: rect.x, y: rect.y, w: rect.width, h: rect.height } });
    }
    return sources;
  });
  evidence.colorSources = colorSources;
  ...
  const visualEffects = await page.evaluate(() => {
    ...
    for (const el of elements) {
      ...
      if (rect.width < 20 || rect.height < 20) continue;
      const tagInfo = el.tagName.toLowerCase() + (el.className ? '.' + el.className.toString().split(' ')[0] : '');
      if (bg.includes('linear-gradient')) effects.push({ type: '线性渐变', value: bg.slice(0, 120), tag: tagInfo });
      else if (bg.includes('radial-gradient')) effects.push({ type: '径向渐变', value: bg.slice(0, 120), tag: tagInfo });
      else if (bg.includes('conic-gradient')) effects.push({ type: '锥形渐变', value: bg.slice(0, 120), tag: tagInfo });
      if (style.backdropFilter && style.backdropFilter !== 'none') effects.push({ type: '毛玻璃', value: style.backdropFilter, tag: tagInfo });
      if (style.mixBlendMode && style.mixBlendMode !== 'normal') effects.push({ type: '混合模式', value: style.mixBlendMode, tag: tagInfo });
      if (style.filter && style.filter !== 'none') effects.push({ type: '滤镜', value: style.filter, tag: tagInfo });
      if (bg.includes('url(') && bg.includes('repeat')) effects.push({ type: '重复纹理/波点', value: bg.slice(0, 80), tag: tagInfo });
      if (effects.length >= 12) break;
    }
    return effects;
  });
  evidence.visualEffects = visualEffects;
